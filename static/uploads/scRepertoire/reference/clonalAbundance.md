...
Displays the number of clones at specific frequencies by sample or
group. Visualization can either be a line graph ( `scale` = FALSE) using
calculated numbers or density plot (`scale` = TRUE). Multiple sequencing
...
A ggplot object visualizing clonal abundance by group, or a data.frame
