...
format:
  pdf:
    ...
    header-includes:
      ...
      - \titleformat{\subsubsection}{\normalsize\itshape\color{sectionblue}}{\thesubsubsection}{0.5em}{}
...
{\small Department of Pathology and Immunology}\\[0.15em]
...
if (length(mbr)) {
  cat(map_chr(mbr, ~ glue("- **{fmt_range(.x$start, .x$end)}**: {.x$org}")), sep = "\n")
} else cat("_(none listed)_")
```
...
if (length(abs)) {
  out <- map_chr(abs, function(a) {
    authors <- fmt_entry(a$authors)
    title <- fix_markup(a$title)
    venue <- nz(a$venue, "")
    year <- nz(a$year, "")
    glue("**{year}**: {authors}. *{title}*. {venue}")
  })
  cat(paste0("- ", out), sep = "\n")
} else {
  cat("_(none listed)_")
...
```{r, results='asis'}
tb <- rev(cv$presentations$textbook) %||% list()

if (length(tb)) {
  out <- map_chr(tb, function(t) {
    authors <- fmt_entry(t$authors)
    title <- fix_markup(t$title)
    isbn <- nz(t$isbn, "")
    year <- nz(t$year, "")
    glue("**{year}**: {authors}. *{title}*. {isbn}")
  })
  cat(paste0("- ", out), sep = "\n")
} else {
  cat("_(none listed)_")
